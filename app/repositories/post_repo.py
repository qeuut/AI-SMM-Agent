...
async def db_set_id_in_user_info(user_id):
    # if not isinstance(user_id, int):
    #     logger.error(f"Функция ---set_user_info--- user_id = {user_id} не является int")
    #     return False # если все ок просто уберу все комментарии по типу такого просто раньше использовал по отдельности каждую из 3 функций а сейчас создал главную
...
async def db_update_session_id(user_id: int) -> bool | str:
    # if not isinstance(user_id, int):
    #     logger.error(f"Функция ---db_reset_session_id--- user_id = {user_id} не является int")
    #     return False, None
...
async def db_get_session_id(user_id: int) -> bool | str:
    # if not isinstance(user_id, int):
    #     logger.error("Функция ---get_session_id--- user_id не int")
    #     return False
