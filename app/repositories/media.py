...
async def change_media_ids(chat_id: int, selected_media_ids: list[str]) -> bool:
    ...
    try:
        await db.execute("UPDATE posts SET selected_media_ids = ? WHERE chat_id = ?", (media_ids_str, chat_id))
        await db.commit()
...
async def get_media_from_db(chat_id: int) -> list | None:
    db = await get_db()
    cursor = await db.execute("SELECT selected_media_ids FROM posts WHERE chat_id = ?", (chat_id,))
